...
const Admin: React.FC = () => {
  ...
  const updateStatus = async (challengeId: number, status: string) => {
    try {
      await api.post(`/admin/challenge/${challengeId}/status`, { status });
              ...
              <tbody className="divide-y divide-slate-800">
                {users.map(u => (
                  <tr key={u.id} className="hover:bg-slate-800/30 transition-colors">
                    ...
                    <td className="p-6">
                      {u.challenge ? (
                        <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${u.challenge.status === 'passed' ? 'bg-emerald-500/20 text-emerald-400' :
